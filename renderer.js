// renderer.js - UI orchestration and IPC wiring for eBay Automation Studio
// This file runs in the renderer process (secure context).
// Heavy operations (fs scan with filtering, DeepSeek API, Playwright CDP) are handled in main.js via IPC.
...
// Auto-detect template from folder name (mirrors main.js guessTemplateFromName)
...
function renderQueue() {
  ...
  if (!filteredQueue.length) {
    if (!queue.length) {
      ...
      queueRows.innerHTML = `
        <div class="empty-state">
          <div class="icon">✅</div>
          <div>No published items yet. Run automation to list products on eBay.</div>
        </div>`;
    } else {
      queueRows.innerHTML = `
        <div class="empty-state">
  ...
  filteredQueue.forEach((item) => {
    ...
    if (item.status === 'Done') {
      actionsHTML = `
        <button class="btn btn-secondary row-btn" data-action="move-back" data-index="${index}">↩️ Move to Queue</button>
    ...
    if (priceIn) {
      ...
      priceIn.addEventListener('input', () => {
        ...
        window.api.saveFolderCustomization(item.name, val, queue[index].template || guessTemplate(item.name));
      });
    }
    ...
    // Move-back handler — just unmark from processed.json, no auto-publish
    const moveBackBtn = row.querySelector('[data-action="move-back"]');
    if (moveBackBtn) {
      moveBackBtn.addEventListener('click', async () => {
        moveBackBtn.disabled = true;
        moveBackBtn.textContent = '...';
        try {
          const result = await window.api.unmarkItem({ name: item.name });
          if (result && result.success) {
            addLog(`[System]: "${item.name}" moved back to product queue.`, 'system');
            await scanCurrentFolder();
          } else {
            addLog(`[Error]: ${result?.message || 'Could not move item'}`, 'system');
            moveBackBtn.disabled = false;
            moveBackBtn.textContent = '↩️ Move to Queue';
          }
        } catch (err) {
...
function setupListeners() {
  // Live logs streamed from main (includes DeepSeek stdout, playwright steps, errors, etc.)
  window.api.onLog((msg) => {
    let type = 'system';
    if (msg.includes('[DeepSeek]') || msg.includes('description') || msg.includes('title')) type = 'grok';
    else if (msg.includes('eBay') || msg.includes('listing') || msg.includes('Playwright')) type = 'marketplace';
  ...
  // Browser (CDP) status - reflects the automation Chrome that Playwright connects to over localhost:9223
  ...
  // Boot message
  setTimeout(() => {
    addLog('Electron + Playwright automation engine initialized. Ready for DeepSeek + CDP flow.', 'system');
  }, 300);
...
async function startAutomation() {
  ...
  addLog(`[System]: Starting full automation (DeepSeek + Playwright CDP) for ${queue.length} items at $${defaultPrice}...`, 'system');

  isRunning = true;
  isPaused = false;
  updateControlStates();

  // This triggers the real workflow in main.js (directory scan + filter + per-item DeepSeek + playwright)
...
function setupControlButtons() {
    ...
    bulkApplyBtn.addEventListener('click', async () => {
      ...
      try {
        for (const item of selectedItems) {
          ...
          const templateToSave = item.template || guessTemplate(item.name);
  ...
  if (imgSaveBtn) {
    imgSaveBtn.addEventListener('click', async () => {
      ...
      // Capture data before closing modal (closeUploadModal clears uploadTargetItem/imagesArray)
      const itemPath = uploadTargetItem.fullPath;
      const images = [...uploadImagesArray];

      // Close modal NOW — don't wait for the slow file save + rescan
      closeUploadModal();

      try {
        const res = await window.api.saveUploadedImages(itemPath, images);
        if (res && res.success) {
          addLog(`[System]: Images saved successfully.`, 'system');
          await scanCurrentFolder();
        } else {
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async () => {
      ...
      const res = await window.api.exportEbayCookies();
  ...
  if (restoreBtn) {
    restoreBtn.addEventListener('click', async () => {
      ...
      const res = await window.api.importEbayCookies();
  ...
  if (marketplaceSelect) {
    marketplaceSelect.addEventListener('change', async () => {
      const newMarketplace = marketplaceSelect.value;
      const res = await window.api.setMarketplace(newMarketplace);
      if (res && res.success) {
        addLog(`[System]: Switched to ${newMarketplace}`, 'system');
        // Re-fetch templates so dropdown shows locale-specific labels
        await fetchTemplates();
        renderQueue();
      } else {
        addLog(`[Error]: Failed to switch marketplace - ${res?.message || 'Unknown error'}`, 'system');
      }
    });
  }
...
async function init() {
  ...
  // Restore saved marketplace
  try {
    const savedMarketplace = await window.api.getMarketplace();
    if (savedMarketplace && marketplaceSelect) {
      marketplaceSelect.value = savedMarketplace;
    }
  } catch (err) {
  ...
  setTimeout(() => {
    ...
    addLog('[Tip]: Use the top-right Automation Chrome buttons: Launch the dedicated profile → log into eBay *once* there. 💾 backs up cookies, ↩︎ restores login, 📁 opens the profile folder (this is what actually keeps you logged in).', 'system');
    addLog('[Tip]: Edit TITLE TEMPLATE (next to price) before Start. Use ${name} placeholder e.g. "${name} 3D Printed DIY Kit".', 'system');
  }, 800);
  ...
  console.log('%c[eBay Automation] Renderer ready. IPC wired to main for DeepSeek + CDP automation.', 'color:#475569');
}
